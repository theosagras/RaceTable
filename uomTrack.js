...
document.getElementById('countryFilter').addEventListener('change', function(evt) {

    countryFilter();
}, false);

function countryFilter() {
    let countryToFilter = document.getElementById('countryFilter').value;
    for (let j = 0; j < tBodyOfMyTable.rows.length; j++) {

        let a = tBodyOfMyTable.rows[j];
        a.style.display = "";
    ...
    if (countryToFilter != '') {
        for (let j = 0; j < tBodyOfMyTable.rows.length; j++) {
            ...
            let a = tBodyOfMyTable.rows[j];
            if (tBodyOfMyTable.rows[j].cells[0].firstElementChild.value != countryToFilter) {
...
function findBestWorstAvg() {
    ...
    for (r = 0; r < tBodyOfMyTable.rows.length; r++) {
        if ((tBodyOfMyTable.rows[r].cells[0].firstElementChild.value == countryToFilter) || (countryToFilter == '')) {
